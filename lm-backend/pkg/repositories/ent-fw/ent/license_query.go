...
type LicenseQuery struct {
	...
	withLicenseProducts *ProductQuery
...
// QueryLicenseProducts chains the current query on the "license_products" edge.
func (lq *LicenseQuery) QueryLicenseProducts() *ProductQuery {
	query := (&ProductClient{config: lq.config}).Query()
...
func (lq *LicenseQuery) QueryOwnerOrg() *OrganizationQuery {
...
func (lq *LicenseQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (lq *LicenseQuery) FirstIDX(ctx context.Context) string {
...
func (lq *LicenseQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (lq *LicenseQuery) OnlyIDX(ctx context.Context) string {
...
func (lq *LicenseQuery) IDs(ctx context.Context) ([]string, error) {
	var ids []string
...
func (lq *LicenseQuery) IDsX(ctx context.Context) []string {
	ids, err := lq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (lq *LicenseQuery) Clone() *LicenseQuery {
	...
	return &LicenseQuery{
		...
		withLicenseProducts: lq.withLicenseProducts.Clone(),
		withOwnerOrg:        lq.withOwnerOrg.Clone(),
...
// WithLicenseProducts tells the query-builder to eager-load the nodes that are connected to
// the "license_products" edge. The optional arguments are used to configure the query builder of the edge.
func (lq *LicenseQuery) WithLicenseProducts(opts ...func(*ProductQuery)) *LicenseQuery {
	query := (&ProductClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	lq.withLicenseProducts = query
	return lq
}
...
func (lq *LicenseQuery) WithOwnerOrg(opts ...func(*OrganizationQuery)) *LicenseQuery {
	query := (&OrganizationClient{config: lq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	lq.withOwnerOrg = query
	return lq
}
...
func (lq *LicenseQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*License, error) {
	var (
		...
		_spec       = lq.querySpec()
		loadedTypes = [2]bool{
			lq.withLicenseProducts != nil,
			lq.withOwnerOrg != nil,
		}
	)
	if lq.withOwnerOrg != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := lq.withLicenseProducts; query != nil {
		if err := lq.loadLicenseProducts(ctx, query, nodes,
			func(n *License) { n.Edges.LicenseProducts = []*Product{} },
			func(n *License, e *Product) { n.Edges.LicenseProducts = append(n.Edges.LicenseProducts, e) }); err != nil {
			return nil, err
		}
	}
	if query := lq.withOwnerOrg; query != nil {
		if err := lq.loadOwnerOrg(ctx, query, nodes, nil,
			func(n *License, e *Organization) { n.Edges.OwnerOrg = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *LicenseQuery) loadLicenseProducts(ctx context.Context, query *ProductQuery, nodes []*License, init func(*License), assign func(*License, *Product)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[string]*License)
	nids := make(map[string]map[*License]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(license.LicenseProductsTable)
		s.Join(joinT).On(s.C(product.FieldID), joinT.C(license.LicenseProductsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(license.LicenseProductsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(license.LicenseProductsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "license_products" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (lq *LicenseQuery) loadOwnerOrg(ctx context.Context, query *OrganizationQuery, nodes []*License, init func(*License), assign func(*License, *Organization)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*License)
	for i := range nodes {
		if nodes[i].organization_licenses == nil {
			continue
		}
		fk := *nodes[i].organization_licenses
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "organization_licenses" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
