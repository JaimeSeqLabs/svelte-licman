...
// ClearIssued clears all "issued" edges to the JwtToken entity.
func (uu *UserUpdate) ClearIssued() *UserUpdate {
	uu.mutation.ClearIssued()
	return uu
}
...
func (uu *UserUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if value, ok := uu.mutation.Claims(); ok {
		_spec.SetField(user.FieldClaims, field.TypeJSON, value)
	}
	if uu.mutation.IssuedCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   user.IssuedTable,
			Columns: []string{user.IssuedColumn},
			Bidi:    false,
	...
	if nodes := uu.mutation.RemovedIssuedIDs(); len(nodes) > 0 && !uu.mutation.IssuedCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   user.IssuedTable,
	...
	if nodes := uu.mutation.IssuedIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   user.IssuedTable,
...
// ClearIssued clears all "issued" edges to the JwtToken entity.
func (uuo *UserUpdateOne) ClearIssued() *UserUpdateOne {
	uuo.mutation.ClearIssued()
	return uuo
}
...
func (uuo *UserUpdateOne) RemoveIssued(j ...*JwtToken) *UserUpdateOne {
	...
	return uuo.RemoveIssuedIDs(ids...)
}
...
func (uuo *UserUpdateOne) sqlSave(ctx context.Context) (_node *User, err error) {
	...
	if nodes := uuo.mutation.RemovedIssuedIDs(); len(nodes) > 0 && !uuo.mutation.IssuedCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   user.IssuedTable,
	...
	if nodes := uuo.mutation.IssuedIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   user.IssuedTable,
