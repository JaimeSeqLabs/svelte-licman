...
func (uc *UserCreate) AddIssuedIDs(ids ...string) *UserCreate {
	uc.mutation.AddIssuedIDs(ids...)
	return uc
}

// AddIssued adds the "issued" edges to the JwtToken entity.
func (uc *UserCreate) AddIssued(j ...*JwtToken) *UserCreate {
	ids := make([]string, len(j))
	for i := range j {
		ids[i] = j[i].ID
	}
	return uc.AddIssuedIDs(ids...)
}
...
func (uc *UserCreate) check() error {
	...
	if _, ok := uc.mutation.Claims(); !ok {
		return &ValidationError{Name: "claims", err: errors.New(`ent: missing required field "User.claims"`)}
	}
	return nil
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.IssuedIDs(); len(nodes) > 0 {
